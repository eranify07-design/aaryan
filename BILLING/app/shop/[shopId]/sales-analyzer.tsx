...
export default function SalesAnalyzerScreen() {
  ...
  const getDailySales = () => {
    const last7 = Array.from({ length: 7 }, (_, i) => {
      ...
      return { label: `${d.getDate()}/${d.getMonth()+1}`, value: dayTotal };
    });
    return last7;
  };
  ...
  const getMonthlySales = () => {
    return Array.from({ length: 12 }, (_, i) => {
      const total = sales.filter((s) => s.createdAt.getFullYear() === now.getFullYear() && s.createdAt.getMonth() === i)
        .reduce((sum, s) => sum + s.totalAmount, 0);
      return { label: MONTHS[i], value: total };
    });
  };
  ...
  const getYearlySales = () => {
    ...
    sales.forEach((s) => {
      ...
      years[y] = (years[y] ?? 0) + s.totalAmount;
    });
    return Object.entries(years).sort(([a], [b]) => Number(a) - Number(b)).map(([year, value]) => ({
      label: year,
      value,
    }));
  };

  const chartData = period === "daily" ? getDailySales() : period === "monthly" ? getMonthlySales() : getYearlySales();
  ...
  const monthTotal = sales.filter((s) => s.createdAt.getFullYear() === now.getFullYear() && s.createdAt.getMonth() === now.getMonth())
    .reduce((sum, s) => sum + s.totalAmount, 0);
  const yearTotal = sales.filter((s) => s.createdAt.getFullYear() === now.getFullYear())
    .reduce((sum, s) => sum + s.totalAmount, 0);
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <LinearGradient colors={["#1A1A2E", "#16213E"]} style={[styles.header, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 12) }]}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>Sales Analyzer</Text>
        </View>
      </LinearGradient>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 24) }]}>
          <View style={styles.statsRow}>
            {[
              { label: "Today", value: `₹${todayTotal.toFixed(0)}`, color: "#FF6B35" },
              { label: "This Month", value: `₹${monthTotal.toFixed(0)}`, color: "#2EC4B6" },
              { label: "This Year", value: `₹${yearTotal.toFixed(0)}`, color: "#667EEA" },
            ].map((stat) => (
          ...
          <View style={[styles.chartCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[styles.chartTitle, { color: colors.text }]}>
              Sales — {period === "daily" ? "Last 7 Days" : period === "monthly" ? now.getFullYear() : "All Time"}
            </Text>

            {chartData.length > 0 ? (
              <BarChart
                data={values}
                labels={chartData.map((d) => String(d.label))}
                color="#FF6B35"
                maxVal={maxVal}
              />
            ) : (
              <View style={styles.noData}>
                ...
                <Text style={[styles.noDataText, { color: colors.textSecondary }]}>No sales data yet</Text>
              </View>
            )}
          </View>
          ...
          <View style={[styles.listCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            {sales.slice(0, 10).map((sale) => (
              <View key={sale.id} style={[styles.saleRow, { borderBottomColor: colors.border }]}>
                <View style={[styles.saleIconBg, { backgroundColor: "#FF6B3520" }]}>
                  <Ionicons name="receipt-outline" size={16} color="#FF6B35" />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.saleName, { color: colors.text }]}>{sale.items.length} item(s)</Text>
            ...
            {sales.length === 0 && (
              <Text style={[styles.noDataText, { color: colors.textSecondary, textAlign: "center", paddingVertical: 24 }]}>No transactions yet</Text>
            )}
          </View>
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  periodTabActive: { backgroundColor: "#FF6B35" },
